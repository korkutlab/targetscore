...
ptCol <- "black"
folder <- "../../../manuscript/figure_targetScore_vs_drugDoseDependence/update/"
...
for(i in 1:length(cellLines)) {
    ...
    filterDat <- filter(dat, negSpCorr > 0.6 & TS >= quantile(dat$TS, 0.9))
    ...
    p <- p + geom_label_repel(data=filterDat, aes(label=displayName), size=2)
