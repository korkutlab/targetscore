...
for(i in 1:nrow(normCoef)) {
    for(j in 1:ncol(normCoef)) {
        t1 <- a2058[i*j, "readout"] / normCoef[i,j]
        t2 <- skmel133[i*j, "readout"] / normCoef[i,j]
...
tmpDrugs <- drugs[!(unique(normA2058[, "treatment"]) %in% excludeDrugs)]
...
#for(curDrug in tmpDrugs) {
    ...
    for(curAntibody in antibodies) {
        ...
        entriesWithCtrl <- cbind(tmpAEntries[allIdx, "readout"], tmpBEntries[allIdx, "readout"])
        colnames(entriesWithCtrl) <- c("A2058", "SkMel133")
        rownames(entriesWithCtrl) <- c("0min", "5min", "15min", "30min", "1hr", "12hr", "24hr", "48hr")
        tmpMat <- t(entriesWithCtrl)
        
        barplot(tmpMat, main=curAntibody, beside=TRUE, col=c("darkblue","red"))
    }
#}
