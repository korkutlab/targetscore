...
mel475_6 <- mel475[which(mel475$time == "6hr"),]
nSamples <- nrow(mel475_6[which(mel475_6$antibody == "Akt"),])
...
y2 <- mel475_ttl_unprtbed_norm[mel475_ttl_unprtbed_norm$replicate == "rep2" & mel475_ttl_unprtbed_norm$treatment != "DMSO", ]
...
rownames(y3) <- "MEKi6_Skmel475"
colnames(y3) <- unique(y2$antibody)
inputFile <- file.path("inst", "melanoma_TS/skmel475", "skmel475_meki_ave_6.txt")
...
inputFile <- file.path("inst", "melanoma_TS/skmel475", paste0(cellLine,"_meki_ave_6.txt"))
