...
repAverage <- function(array1) {
  ...
  average_rep$readout <- (array_rep1$readout+array_rep2$readout+array_rep3$readout+array_rep4$readout)/4
  nall <- nrow(array1)
  nuniq <- nall*0.25

for (i in 1:nuniq){
  average_rep[i,]$cv <- sd(c(array_rep1[i,]$readout,array_rep2[i,]$readout,array_rep3[i,]$readout,array_rep4[i,]$readout))  
  
}

#    stdev <- sd(c(array_rep1$readout,array_rep2$readout,array_rep3$readout,array_rep4$readout))
