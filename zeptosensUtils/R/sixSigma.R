...
#' @concept zeptosens
#' @export
sixSigma <- function(arrayData, baseTreatment, nsamp, antibodyNum, sixsigname) {
  arrayData$readout <- log2(abs(arrayData$readout))

  baseRead <- matrix(0, nrow = length(antibodyNum), ncol = nsamp)
  nnorm <- matrix(0, nrow = length(antibodyNum), ncol = 1)
  base6Sigma <- matrix(0, nrow = length(antibodyNum), ncol = 1)
  ...
  for (i in 1:length(antibodyNum)) {
    ...
    base6Sigma[i, 1] <- 6 * sd(baseRead[i, ], na.rm = T)
    t1 <- (i - 1) * nsamp + 1
    t2 <- i * nsamp
    array1 <- arrayData[t1:t2, ]
    maxrdt[i, 1] <- max(abs(array1[, "readout"]))

    print(paste(maxrdt[i, 1], base6Sigma[i, 1]))
    # print((i-1)*nsamp+1) print(i*nsamp)
    if (maxrdt[i, 1] > base6Sigma[i, 1]) {
