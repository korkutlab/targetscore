...
plotZeptosensDataSingleAntibodyAllDrugs <- function(normArray1, normArray2, antibodies,
                                                    excludeTreatments, ctrlTreatment, 
                                                    ctrlTime, plotColNames, plotRowNames, 
                                                    plotColors, plotDir, asPdf = TRUE,
                                                    plotPrefix = NULL) {
  ...
  for (curAntibody in antibodies) {
    ...
    for (curDrug in tmpDrugs) {
      ...
      idx <- intersect(
        which(tmpArray1Entries[, "antibody"] == curAntibody),
        which(tmpArray1Entries[, "treatment"] == curDrug)
      )

      t0Idx <- intersect(
        which(tmpArray1Entries[, "treatment"] == ctrlTreatment),
    ...
    plot(c(1, length(tmpDrugs)), range(tmpMat), main = paste0(
      curAntibody, " Sample: ",
      plotColNames
    ...
    for (i in 1:length(tmpDrugs)) {
      curDrug <- tmpDrugs[i]
      lines(1:nrow(tmpMat), tmpMat[, curDrug],
        type = "b", lwd = 1.5, col = plotColors[i],
        pch = plotChar[i]
      )
    }
    ...
    legend("topright",
      legend = tmpDrugs, lty = rep(1, length(tmpDrugs)), pch = plotChar,
