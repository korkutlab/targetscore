...
#' @concept zeptosens
#' @export
plotZeptosensDataSingleAntibodyAllDrugs <- function(normArray1, normArray2, antibodies, 
    excludeTreatments, ctrlTreatment, ctrlTime, plotColNames, plotRowNames, plotColors, 
    plotDir, asPdf = TRUE, plotPrefix = NULL) {
    ...
    for (curAntibody in antibodies) {
        ...
        # par(mfrow=c(plotRows, plotCols)) par(mar=c(2,2,1.5,1), oma=c(0,0,2,0))
        par(mar = c(5.1, 4.1, 4.1, 8.1), xpd = TRUE)
        
        # Setup plot type='n' for nothing
        plot(c(1, length(tmpDrugs)), range(tmpMat), main = paste0(curAntibody, " Sample: ", 
            plotColNames), xaxt = "n", xlab = "Time", ylab = "RFI", type = "n")
        ...
        for (i in 1:length(tmpDrugs)) {
            curDrug <- tmpDrugs[i]
            lines(1:nrow(tmpMat), tmpMat[, curDrug], type = "b", lwd = 1.5, col = plotColors[i], 
                pch = plotChar[i])
        }
        
        # plot(tmpMat, main=curAntibody, col='blue') title(main=curAntibody, outer=TRUE)
        legend("topright", legend = tmpDrugs, lty = rep(1, length(tmpDrugs)), pch = plotChar, 
            lwd = rep(2.4, length(tmpDrugs)), col = plotColors, inset = c(-0.2, 0), 
            title = "Treatment")
