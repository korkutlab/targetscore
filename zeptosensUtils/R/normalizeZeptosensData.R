...
#' @concept zeptosens
#' @export
normalizeZeptosensData <- function(array1, array2, totalProteinLevels1, totalProteinLevels2, 
    controlProbeIndex, antibodyNum) {
    ratios <- NULL
    ...
    # write.table(ratios, file='inst/dataInst/a2058_skmel133_ctrlRatios.txt',
    # sep='\t', col.names=TRUE, row.names=FALSE, quote=FALSE)
    
    normCoef1 <- (ratios[, "ratios"]/ratios[, "ratios"]) %*% t(totalProteinLevels1[, 
        3])
    normCoef2 <- (ratios[, "ratios"]/ratios[, "ratios"]) %*% t(totalProteinLevels2[, 
        3])
