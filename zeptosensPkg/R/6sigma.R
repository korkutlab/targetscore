...
sixsigma <- function(arrayData,base_treatment,nsamp,antibodyNum,sixsigname) {
    ...
    maxrdt <-  matrix(0,nrow=length(antibodyNum),ncol=1)
                  ...
                  write.table(col6sigma,file=sixsigname,row.names=F,col.names=F,sep="\t",quote=F)
...
#           print(arrayData[(i-1)*nsamp+j,]) 
        ...
        if(maxrdt[i,1] > base_6sigma[i,1]) {
            write.table(paste(arrayData[(i-1)*nsamp+1,"antibody"],maxrdt[i,1],base_6sigma[i,1],sep="\t"),file=sixsigname,append=T,quote=F,row.names=F,col.names=F)
    }
